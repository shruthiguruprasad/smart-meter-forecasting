    ...
    def prepare_driver_analysis_data(df: pd.DataFrame, 
        ...
        # Get feature columns (exclude IDs, dates, and target-related columns)
        exclude_patterns = [
            "LCLid", "day", "holiday_type",  # Basic exclusions
            target_col,  # Target variable
            f"lag1_{target_col}", f"lag7_{target_col}",  # Target lags
            ...
            f"pct_change_{target_col}"  # Target percentage changes
        ]
        ...
        for col in df.columns:
            ...
            if any(pattern in col for pattern in exclude_patterns):
                continue
            # Skip target column and its derivatives
            if col == target_col or col.startswith(f"{target_col}_"):
                continue
            feature_cols.append(col)
        ...
        # Define feature groups for consumption driver analysis
        half_hourly = [c for c in feature_cols if c.startswith('hh_')]
        consumption = [c for c in feature_cols if any(x in c for x in ['kwh', 'load', 'peak', 'base'])]
        temporal = [c for c in feature_cols if any(x in c for x in ['dayofweek', 'month', 'season', 'quarter', 'holiday'])]
        weather = [c for c in feature_cols if any(x in c for x in ['temp', 'heating', 'cooling', 'humidity', 'wind', 'cloud'])]
        socio_economic = [c for c in feature_cols if any(x in c for x in ['acorn', 'Acorn'])]
        time_series = [c for c in feature_cols if any(x in c for x in ['lag', 'roll', 'delta', 'pct_change'])]
        
        # Get features that don't belong to any other group
        assigned_features = set(half_hourly + consumption + temporal + weather + socio_economic + time_series)
